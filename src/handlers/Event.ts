...
export default async (portal: BedrockPortal, event: EventResponse) => {
  ...
  const sessionMembers = Object.values(session.members).filter(member => member.constants.system.xuid !== portal.sessionOwner?.xuid)

  const xuids = sessionMembers.map(e => e.constants.system.xuid)

  const profiles = await portal.rest.getProfiles(xuids)
